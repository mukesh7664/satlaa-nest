...
export class AuthService {
    ...
    async login(customer: any) {
        const payload = { email: customer.email, sub: customer.id, customerId: customer.id, role: 'customer' };
        return {
    ...
    async register(customerDto: any) {
        ...
        this.emailService.sendWelcomeEmail(customer.email, customer.name).catch(err => this.logger.error('Failed to send welcome email', err));
    ...
    async updateProfile(userId: string, data: any) {
        ...
        const updated = await this.customersService.update(userId, updateData);
        const { password, ...result } = updated;
        return result;
    }

    async changePassword(userId: string, currentPass: string, newPass: string) {
        const customer = await this.customersService.findOneById(userId);
        ...
        await this.customersService.update(userId, { password: hashedPassword });
