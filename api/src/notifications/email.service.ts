...
export class EmailService {
  ...
  constructor(
    ...
    this.globalFromName = this.configService.get<string>('EMAIL_FROM_NAME') || 'Fanostyle';
    this.globalFromEmail = this.configService.get<string>('EMAIL_FROM_ADDRESS') || 'noreply@fanostyle.in';
  ...
  async sendOrderConfirmationEmail(order: any, store?: any) {
    ...
    const storeName = store?.name || 'Fanostyle';
    if (!email) {
  ...
  async sendOtpEmail(email: string, otp: string, name?: string): Promise<void> {
    ...
    const subject = `${otp} is your Fanostyle verification code`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333; line-height: 1.6;">
        <h2 style="color: #4F46E5; border-bottom: 2px solid #E5E7EB; padding-bottom: 10px;">Account Verification - Fanostyle</h2>
        <p>Dear ${displayName},</p>
        <p>We received a request to reset the password for your Fanostyle account associated with <strong>${email}</strong>.</p>
        ...
        <br />
        <p>Best regards,</p>
        <p><strong>The Fanostyle Team</strong></p>
      </div>
    `;
  ...
  async sendTemplateTestEmail(templateId: string, targetEmail: string): Promise<void> {
    ...
    const siteName = this.globalFromName || 'Fanostyle Test';
