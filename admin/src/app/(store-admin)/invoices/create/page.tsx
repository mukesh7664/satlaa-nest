...
import {
  ArrowLeft, Plus, Trash2, FileText, User, Package, ChevronRight, Send, Save, Receipt,
} from "lucide-react";
...
export default function CreateInvoicePage() {
  ...
  const addItem = () => {
    ...
    if (!isCustom) {
      if (!selectedProduct) { toast.error("Select a valid product from the list"); return; }
    }
  ...
  const handleSubmit = async (sendNow = false) => {
    try {
      ...
      const invoiceData = {
        customer: formData.customer, customerId: formData.customerId, items: formData.items,
        pricing, dueDate: formData.dueDate, notes: formData.notes, invoiceDate: new Date().toISOString(),
      };
      const token = localStorage.getItem("token");
      const res = await axios.post(`${API_URL}/admin/invoices`, invoiceData, { headers: { Authorization: `Bearer ${token}` } });
      const invoiceId = res.data.data.id;
      if (sendNow) {
        await axios.post(`${API_URL}/admin/invoices/${invoiceId}/send`, {}, { headers: { Authorization: `Bearer ${token}` } });
        toast.success("Invoice created and sent!");
      } else {
        toast.success("Invoice created successfully!");
      }
      router.push("/invoices");
    } catch { toast.error("Failed to create invoice"); } finally { setLoading(false); }
  };
    ...
    <div className="p-6 space-y-6 font-sans">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Create and send a professional invoice to your customer</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => router.push("/invoices")} className="px-4 py-2 text-sm font-medium border rounded-lg hover:bg-slate-50 transition-all" style={{ borderColor: btnColor, color: btnColor }}>Cancel</button>
          <button onClick={() => handleSubmit(false)} disabled={loading} className="px-4 py-2 text-sm font-medium border rounded-lg transition-all disabled:opacity-60 flex items-center gap-1.5" style={{ borderColor: btnColor, color: btnColor }}>
        ...
        <div className="space-y-6 lg:sticky lg:top-6 h-fit">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="px-5 pb-5 space-y-2">
              ...
              <button onClick={() => router.push("/invoices")} className="w-full py-2.5 text-sm font-medium transition-all" style={{ color: btnColor }}>Cancel</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-slate-700">{new Date(formData.dueDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Status</span>
                <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-700">Draft</span>
              </div>
            </div>
          </div>
        </div>
