...
import {
  ArrowLeft, Plus, Trash2, FileText, User, Package, ChevronRight, Send, Save,
} from "lucide-react";
...
export default function CreateEstimatePage() {
  ...
  const handleSubmit = async (sendNow = false) => {
    try {
      ...
      const estimateData = { customer: formData.customer, customerId: formData.customerId, items: formData.items, pricing, validUntil: formData.validUntil, notes: formData.notes };
      const token = localStorage.getItem("token");
      const res = await axios.post(`${API_URL}/admin/estimates`, estimateData, { headers: { Authorization: `Bearer ${token}` } });
      const estimate = res.data.data || res.data;
      if (!estimate.id) throw new Error("Could not retrieve estimate ID");
      if (sendNow) {
        await axios.post(`${API_URL}/admin/estimates/${estimate.id}/send`, {}, { headers: { Authorization: `Bearer ${token}` } });
        toast.success("Estimate created and sent!");
      } else {
        toast.success("Estimate created!");
      }
      router.push(`/estimates/${estimate.id}`);
    } catch { toast.error("Failed to create estimate"); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="p-6 space-y-6 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Create Estimate</h1>
          <p className="text-sm text-slate-500 mt-1">Generate a price estimate or quote for your customer</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => router.push("/estimates")} className="px-4 py-2 text-sm font-medium border rounded-lg hover:bg-slate-50 transition-all" style={{ borderColor: btnColor, color: btnColor }}>Cancel</button>
          <button onClick={() => handleSubmit(false)} disabled={loading} className="px-4 py-2 text-sm font-medium border rounded-lg transition-all disabled:opacity-60" style={{ borderColor: btnColor, color: btnColor }}>
            <span className="flex items-center gap-1.5"><Save size={14} />{loading ? "Saving..." : "Save Draft"}</span>
          </button>
          <button onClick={() => handleSubmit(true)} disabled={loading} className="px-4 py-2 text-sm font-semibold text-white rounded-lg transition-all hover:opacity-90 disabled:opacity-60 flex items-center gap-1.5" style={{ background: btnColor }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 bg-slate-50/50">
              <div className="w-8 h-8 rounded-lg bg-purple-50 flex items-center justify-center"><FileText size={16} className="text-purple-600" /></div>
              <h2 className="text-sm font-semibold text-slate-700">Estimate Details</h2>
            </div>
            <div className="p-5 grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <TextField fullWidth size="small" multiline rows={3}
                  ...
                  placeholder="Add terms, conditions or remarks..."
                />
              </div>
            </div>
          </div>
            ...
            <div className="p-5 space-y-4">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        {["Item","HSN","Qty","Price","Disc","Tax","Total",""].map((h, i) => (
              ...
              {formData.items.length === 0 && (
                <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                  ...
                  <p className="text-xs mt-1">Add products from catalog or enter custom items above</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-6 h-fit">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50">
              <h2 className="text-sm font-semibold text-slate-700">Estimate Summary</h2>
            </div>
            <div className="p-5 space-y-3">
              ...
              <div className="flex justify-between text-sm text-slate-600"><span>Tax</span><span className="font-medium text-slate-800">₹{pricing.tax.toFixed(2)}</span></div>
              ...
              <p className="text-xs text-slate-400">{formData.items.length} item{formData.items.length !== 1 ? "s" : ""} · Valid until {formData.validUntil}</p>
            </div>
            <div className="px-5 pb-5 space-y-2">
              <button onClick={() => handleSubmit(true)} disabled={loading} className="w-full py-2.5 text-sm font-semibold text-white rounded-lg hover:opacity-90 disabled:opacity-60 flex items-center justify-center gap-2" style={{ background: btnColor }}>
                <Send size={14} />{loading ? "Sending..." : "Create & Send"}
              </button>
              <button onClick={() => handleSubmit(false)} disabled={loading} className="w-full py-2.5 text-sm font-medium border rounded-lg hover:bg-slate-50 transition-all flex items-center justify-center gap-2" style={{ borderColor: btnColor, color: btnColor }}>
                <Save size={14} />Save as Draft
              </button>
              <button onClick={() => router.push("/estimates")} className="w-full py-2.5 text-sm font-medium transition-all" style={{ color: btnColor }}>Cancel</button>
            </div>
          </div>
        </div>
      </div>
