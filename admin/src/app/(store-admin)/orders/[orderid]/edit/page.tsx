...
import {
  ArrowLeft, Plus, Trash2, FileText, User, Package, ChevronRight, ShoppingCart, Loader2,
} from "lucide-react";
...
export default function EditOrderPage() {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/admin/orders/${params.orderid}`,
        {
      ...
      setFormData({
        ...
        customer: {
          ...
          shippingAddress: {
            street: order.shippingAddress?.street || order.billingAddress?.street || "",
            city: order.shippingAddress?.city || order.billingAddress?.city || "",
            state: order.shippingAddress?.state || order.billingAddress?.state || "",
            country: order.shippingAddress?.country || order.billingAddress?.country || "India",
            pincode: order.shippingAddress?.pincode || order.billingAddress?.pincode || "",
          },
        },
        items: order.items,
        ...
        status: order.status,
        notes: order.notes || "",
        additionalDiscount: order.pricing?.additionalDiscount || 0,
  ...
  const calculatePricing = () => {
    ...
    // Total Discount
    const totalDiscount = itemDiscounts + additionalDiscount + discountAmount;
  ...
  const handleProductSelect = (product: ProductOption | null) => {
    ...
    const basePrice =
      product.simplePricing?.discountedPrice ||
      product.simplePricing?.basePrice ||
      product.simplePricing?.minPrice ||
      0;
    ...
    setNewItem({
      ...newItem,
      productName: product.productInfo?.title || product.name,
      price: basePrice,
      tax: taxAmount,
      hsn: product.productInfo?.hsn_code || product.hsn || "",
    });
  };

  const addItem = () => {
    if (!newItem.productName || newItem.price <= 0) {
    ...
    try {
      ...
      const orderData = {
        ...
        billingAddress: {
          ...
          street: formData.customer.billingAddress?.street || "",
          city: formData.customer.billingAddress?.city || "",
          state: formData.customer.billingAddress?.state || "",
          country: formData.customer.billingAddress?.country || "India",
          pincode: formData.customer.billingAddress?.pincode || "",
        },
        shippingAddress: {
          fullName: formData.customer.name,
          email: formData.customer.email,
          phone: formData.customer.phone,
          street: formData.customer.shippingAddress?.street || "",
          city: formData.customer.shippingAddress?.city || "",
          state: formData.customer.shippingAddress?.state || "",
          country: formData.customer.shippingAddress?.country || "India",
          pincode: formData.customer.shippingAddress?.pincode || "",
        },
      ...
      const token = localStorage.getItem("token");
      await axios.put(`${API_URL}/admin/orders/${params.orderid}`, orderData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 border rounded-lg text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-all border-slate-200"
          >
            <ArrowLeft size={16} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Edit Order</h1>
            <p className="text-sm text-slate-500 mt-1">Update the details below for this manual order</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              ...
              {formData.discountAmount > 0 && (
                <div className="flex justify-between text-sm text-slate-600">
                  <span>Coupon Discount {formData.discountCode ? `(${formData.discountCode})` : ""}</span>
                  <span className="font-medium text-emerald-600">-₹{formData.discountAmount.toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between text-sm text-slate-600">
                <span>Discount</span>
              ...
              <div className="flex justify-between text-sm text-slate-600">
                <span>Shipping</span>
                <span className="font-medium text-slate-800">₹{pricing.shippingCharges.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm text-slate-600">
                <span>Tax (18%)</span>
